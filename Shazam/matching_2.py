...
# ---------- small helpers for saving/loading per-song hashes ----------
...
# ---------- DEMO (minimal changes from your original) ----------
if __name__ == "__main__":
    ...
    songs = {
        ...
        "song7": "music/Bekhayali.mp3",
        "song8": "music/Main_Rahoon_Ya_Na_Rahoon.mp3",
        "song9": "music/Pachtaoge.mp3",
        "song10": "music/Tum_Hi_Ho.mp3",
        "song11": "music/Tujhe_Kitna_Chahein_Aur.mp3",
        # "song7": "recordings/trim_dheere.mp3"
    }

    # load any previously saved per-song hashes
    saved_hashes = load_hash_db()

    # Index songs (skip hashing if already saved)
    for song_id, path in songs.items():
        if song_id in saved_hashes:
            # use saved hashes to populate FingerprintDB without re-hashing audio
            db.add_song(song_id, saved_hashes[song_id])
            print(f"Loaded saved hashes for {song_id} ({len(saved_hashes[song_id])} hashes)")
            continue

        # else compute hashes and save them
        try:
        ...
        # convert hashes to JSON-serializable structure and store under song_id
        saved_hashes[song_id] = []
        for h, t in hashes:
            # ensure h is a tuple of simple numbers (it already is in generate_hashes)
            saved_hashes[song_id].append((tuple(h), float(t)))

        # persist immediately so partial progress is saved
    ...
    try:
        ...
        print(f"Failed to load query {query_path}: {e}")
        query_sig = None
        sr = None

    if query_sig is not None:
        peak = np.max(np.abs(query_sig)) if query_sig.size else 0.0
        if peak > 0:
            query_sig = query_sig / peak
        else:
            print(f"Warning: {query_path} appears silent, skipping query.")
            query_sig = None
    ...
    if query_sig is not None:
        ...
        # Or save to mp4/gif
        # animate_constellation(query_sig, sr, const_map, hashes, save_path="constellation.mp4")
